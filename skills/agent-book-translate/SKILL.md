...
- `OPENAI_API_KEY` (or custom provider key): Your LLM API bearer token.
...
3. **Concurrency & Rate Limits:** While a concurrency limit of `3` is functional, high RPM API limits (like `gpt-4o-mini` under multiple parallel jobs) can easily cause rate limiting. A single-job **`concurrency = 2`** is a highly recommended and safe default. Adjust concurrency according to your API provider's RPM limit to minimize retries and optimize overall speed.
